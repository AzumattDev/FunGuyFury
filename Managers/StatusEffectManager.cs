...
public class CustomSE
{
    ...
    [Description(
        "Set the type of effect you are adding. If the item can attack, consume, or be equipped, change this to correspond.")]
    public EffectType Type;
    ...
    [Description(
        "Adds the CustomSE to a prefab by adding it on ZNetScene awake. The effect type will be used to determine when the effect applies.")]
    ...
    private static object? configSync
    {
        get
        {
            if (_configSync == null && hasConfigSync)
            {
                ...
                {
                    _configSync =
                        Activator.CreateInstance(configSyncType, plugin.Info.Metadata.GUID + " StatusEffectManager");
...
public static class EffectManager
{
    static EffectManager()
    {
        ...
        harmony.Patch(AccessTools.DeclaredMethod(typeof(Localization), nameof(Localization.SetupLanguage)),
            postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(CustomSE.LocalizationCache),
                nameof(CustomSE.LocalizationCache.LocalizationPostfix))));
    }
    ...
    {
        SE_Berserk customSE = (SE_Berserk)assets.LoadAsset<ScriptableObject>(customEffectName);
