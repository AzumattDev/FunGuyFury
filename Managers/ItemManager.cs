                ...
                if ((item.configurability & Configurability.Stats) != 0)
                {
                        ...
                        Action<ItemDrop.ItemData.SharedData, T> setValue)
                    {
                        ...
                        cfg.SettingChanged += (_, _) =>
                        {
                            ...
                            Inventory[] inventories = Player.m_players.Select(p => p.GetInventory())
                        ...
                        or ItemDrop.ItemData.ItemType.TwoHandedWeapon or ItemDrop.ItemData.ItemType.Bow)
                    {
                        ...
                        if (itemType is ItemDrop.ItemData.ItemType.Bow)
                        {
                            ...
                            statcfg("Maximum Draw Time", $"Time until {englishName} is fully drawn at skill level 0.",
                                shared => shared.m_holdDurationMin,
                                (shared, value) => shared.m_holdDurationMin = value);
                            statcfg("Stamina Drain", $"Stamina drain per second while drawing {englishName}.",
                                shared => shared.m_holdStaminaDrain,
                                (shared, value) => shared.m_holdStaminaDrain = value);
                        }
                    }
                }
            }
